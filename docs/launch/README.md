...
**관리자 메뉴 후속: [CMS·정산 조회·서브관리자 권한 복구](admin-console-rollout.md).** 가상 계정만 보이던 기본 화면을 운영 대시보드와 11개 업무 메뉴로 구성했다. 실제 등록 데이터 조회와 재인증을 통한 기존 서브관리자 권한 변경을 별도 계정 서비스에 연결한다. 정산 승인·지급과 전체 콘텐츠 조치 활성화는 별도 범위다. 적용 여부는 연결 문서의 검증 산출물로 확인한다.
