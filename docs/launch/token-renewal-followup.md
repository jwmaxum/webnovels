...
| 현재 브라우저 권한 | 민감 열 SELECT 및 39개 public 테이블의 쓰기 권한이 허용됨. 실제 쓰기 요청은 보내지 않음 |
...
4. [유지보수 권한 차단 SQL](../../database/p0/003_maintenance_containment.sql)을 준비했다. P0 잠금 완료 표시나 기능 활성화를 하지 않으며, 기존 행을 바꾸지 않는다.
...
영구 적용은 자동 승인 검토에서 거절됐다. 사유는 public 테이블·시퀀스·함수 권한과 RLS를 넓게 제한하면 기존 서비스 접근이 중단될 수 있어, 일반적인 서비스 실행 요청만으로 이 구체적 변경을 승인했다고 볼 수 없다는 것이다. 사용자에게 다음 효과를 설명하고 별도 승인을 요청했다.
...
승인이 확인되기 전 SQL을 우회 실행하거나 범위를 바꿔 같은 효과를 적용하지 않는다. 전체 서비스 전환에는 아래 데이터·계정 작업도 필요하다.
