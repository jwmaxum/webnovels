...
ALTER TABLE public.readers ADD COLUMN IF NOT EXISTS username TEXT;
ALTER TABLE public.readers ADD COLUMN IF NOT EXISTS nickname TEXT;
ALTER TABLE public.readers ADD COLUMN IF NOT EXISTS email TEXT;
ALTER TABLE public.readers ADD COLUMN IF NOT EXISTS phone TEXT;
ALTER TABLE public.readers ADD COLUMN IF NOT EXISTS subscription_status TEXT DEFAULT '일반 회원';
ALTER TABLE public.readers ADD COLUMN IF NOT EXISTS is_adult_verified BOOLEAN DEFAULT false;
ALTER TABLE public.readers ADD COLUMN IF NOT EXISTS adult_verified_at TIMESTAMPTZ;
ALTER TABLE public.readers ADD COLUMN IF NOT EXISTS status TEXT DEFAULT 'ACTIVE';
...
ALTER TABLE public.authors ADD COLUMN IF NOT EXISTS auth_user_id UUID;
ALTER TABLE public.authors ADD COLUMN IF NOT EXISTS username TEXT;
ALTER TABLE public.authors ADD COLUMN IF NOT EXISTS pen_name TEXT;
ALTER TABLE public.authors ADD COLUMN IF NOT EXISTS profile_image TEXT;
ALTER TABLE public.authors ADD COLUMN IF NOT EXISTS bio TEXT;
ALTER TABLE public.authors ADD COLUMN IF NOT EXISTS status TEXT DEFAULT 'PENDING';
ALTER TABLE public.authors ADD COLUMN IF NOT EXISTS verified_at TIMESTAMPTZ;
...
CREATE TABLE IF NOT EXISTS public.episode_unlocks (
  ...
  user_id UUID,
  episode_id BIGINT,
  unlock_type public.unlock_type NOT NULL DEFAULT 'REWARDED_AD',
...
ALTER TABLE public.author_settlements ADD COLUMN IF NOT EXISTS author_id BIGINT;
ALTER TABLE public.author_settlements ADD COLUMN IF NOT EXISTS author_name_snapshot TEXT;
...
ALTER TABLE public.comments ADD COLUMN IF NOT EXISTS nickname_snapshot TEXT;
ALTER TABLE public.comments ADD COLUMN IF NOT EXISTS work_id BIGINT;
ALTER TABLE public.comments ADD COLUMN IF NOT EXISTS episode_id BIGINT;
...
ALTER TABLE public.comments ADD COLUMN IF NOT EXISTS is_blocked BOOLEAN DEFAULT false;
...
ALTER TABLE public.content_reviews ADD COLUMN IF NOT EXISTS work_id BIGINT;
ALTER TABLE public.content_reviews ADD COLUMN IF NOT EXISTS episode_id BIGINT;
ALTER TABLE public.content_reviews ADD COLUMN IF NOT EXISTS work_title_snapshot TEXT;
ALTER TABLE public.content_reviews ADD COLUMN IF NOT EXISTS author_name_snapshot TEXT;
ALTER TABLE public.content_reviews ADD COLUMN IF NOT EXISTS status public.review_status DEFAULT 'PENDING';
...
CREATE POLICY p_public_works ON public.works FOR SELECT TO public
USING (status IN ('PUBLISHED', 'ONGOING', 'COMPLETED'));

DROP POLICY IF EXISTS p_public_episodes ON public.episodes;
CREATE POLICY p_public_episodes ON public.episodes FOR SELECT TO public
USING (status = 'PUBLISHED');

DROP POLICY IF EXISTS p_public_authors ON public.authors;
CREATE POLICY p_public_authors ON public.authors FOR SELECT TO public
USING (status = 'APPROVED');

DROP POLICY IF EXISTS p_public_comments ON public.comments;
CREATE POLICY p_public_comments ON public.comments FOR SELECT TO public
USING (is_deleted = false AND is_blocked = false);

DROP POLICY IF EXISTS p_public_fan_meetings ON public.fan_meetings;
CREATE POLICY p_public_fan_meetings ON public.fan_meetings FOR SELECT TO public
USING (status = 'OPEN');

DROP POLICY IF EXISTS p_public_goods ON public.goods;
CREATE POLICY p_public_goods ON public.goods FOR SELECT TO public
USING (status = 'ON_SALE');

DROP POLICY IF EXISTS p_public_platform_stats ON public.platform_stats;
CREATE POLICY p_public_platform_stats ON public.platform_stats FOR SELECT TO public
USING (true);

DROP POLICY IF EXISTS p_public_system_config ON public.system_config;
CREATE POLICY p_public_system_config ON public.system_config FOR SELECT TO public
USING (true);
...
CREATE POLICY p_admin_all_readers ON public.readers FOR ALL TO authenticated
USING ((SELECT private.is_admin())) WITH CHECK ((SELECT private.is_admin()));

DROP POLICY IF EXISTS p_admin_all_authors ON public.authors;
CREATE POLICY p_admin_all_authors ON public.authors FOR ALL TO authenticated
USING ((SELECT private.is_admin())) WITH CHECK ((SELECT private.is_admin()));

DROP POLICY IF EXISTS p_admin_all_private_profiles ON public.author_private_profiles;
CREATE POLICY p_admin_all_private_profiles ON public.author_private_profiles FOR ALL TO authenticated
USING ((SELECT private.is_admin())) WITH CHECK ((SELECT private.is_admin()));

DROP POLICY IF EXISTS p_admin_all_accounts ON public.author_settlement_accounts;
CREATE POLICY p_admin_all_accounts ON public.author_settlement_accounts FOR ALL TO authenticated
USING ((SELECT private.is_admin())) WITH CHECK ((SELECT private.is_admin()));

DROP POLICY IF EXISTS p_admin_all_works ON public.works;
CREATE POLICY p_admin_all_works ON public.works FOR ALL TO authenticated
USING ((SELECT private.is_admin())) WITH CHECK ((SELECT private.is_admin()));

DROP POLICY IF EXISTS p_admin_all_episodes ON public.episodes;
CREATE POLICY p_admin_all_episodes ON public.episodes FOR ALL TO authenticated
USING ((SELECT private.is_admin())) WITH CHECK ((SELECT private.is_admin()));

DROP POLICY IF EXISTS p_admin_all_contents ON public.episode_contents;
CREATE POLICY p_admin_all_contents ON public.episode_contents FOR ALL TO authenticated
USING ((SELECT private.is_admin())) WITH CHECK ((SELECT private.is_admin()));

DROP POLICY IF EXISTS p_admin_all_panels ON public.episode_panels;
CREATE POLICY p_admin_all_panels ON public.episode_panels FOR ALL TO authenticated
USING ((SELECT private.is_admin())) WITH CHECK ((SELECT private.is_admin()));

DROP POLICY IF EXISTS p_admin_all_reviews ON public.content_reviews;
CREATE POLICY p_admin_all_reviews ON public.content_reviews FOR ALL TO authenticated
USING ((SELECT private.is_admin())) WITH CHECK ((SELECT private.is_admin()));

DROP POLICY IF EXISTS p_admin_all_reports ON public.reports;
CREATE POLICY p_admin_all_reports ON public.reports FOR ALL TO authenticated
USING ((SELECT private.is_admin())) WITH CHECK ((SELECT private.is_admin()));

DROP POLICY IF EXISTS p_admin_all_ad_units ON public.ad_units;
CREATE POLICY p_admin_all_ad_units ON public.ad_units FOR ALL TO authenticated
USING ((SELECT private.is_admin())) WITH CHECK ((SELECT private.is_admin()));

DROP POLICY IF EXISTS p_admin_all_ad_events ON public.ad_events;
CREATE POLICY p_admin_all_ad_events ON public.ad_events FOR ALL TO authenticated
USING ((SELECT private.is_admin())) WITH CHECK ((SELECT private.is_admin()));

DROP POLICY IF EXISTS p_admin_all_revenue_periods ON public.revenue_periods;
CREATE POLICY p_admin_all_revenue_periods ON public.revenue_periods FOR ALL TO authenticated
USING ((SELECT private.is_admin())) WITH CHECK ((SELECT private.is_admin()));

DROP POLICY IF EXISTS p_admin_all_revenue_ledger ON public.revenue_ledger;
CREATE POLICY p_admin_all_revenue_ledger ON public.revenue_ledger FOR ALL TO authenticated
USING ((SELECT private.is_admin())) WITH CHECK ((SELECT private.is_admin()));

DROP POLICY IF EXISTS p_admin_all_settlements ON public.author_settlements;
CREATE POLICY p_admin_all_settlements ON public.author_settlements FOR ALL TO authenticated
USING ((SELECT private.is_admin())) WITH CHECK ((SELECT private.is_admin()));
